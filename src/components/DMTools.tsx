...
import { Wand2, UserPlus, Coins, Lightbulb, Send, Copy, X, Loader2, BookPlus, Image as ImageIcon, Lock, Unlock, Target, MessageSquare, CloudRain, CloudSnow, CloudFog, Sun, Flame, Camera, ScanEye, Upload, TrendingUp, TrendingDown, Minus } from 'lucide-react';
...
type GenMode = 'npc' | 'loot' | 'hook' | 'item_art' | 'handout' | 'control' | 'vision';
...
const DMTools: React.FC<DMToolsProps> = ({ 
  ...
  const { uploadMap, getPartyMomentum } = useGameStore();
  const momentum = getPartyMomentum();
  ...
  const handleMapUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      onSendToChat("Завантажую скан мапи... ШІ аналізує структуру...", undefined);
  ...
  const getTitle = () => {
      ...
      if (mode === 'vision') return "Мапа з Фото (Vision 2.0)";
      return "Generated";
  };
  ...
  const getMomentumUI = () => {
      if (momentum === 'struggling') {
          return (
              <div className="flex items-center gap-2 text-xs bg-red-900/30 border border-red-800 text-red-300 p-2 rounded w-full justify-center">
                  <TrendingDown className="w-4 h-4" />
                  <span>Struggling (Fail Streak) - AI is helping</span>
              </div>
          );
      }
      if (momentum === 'dominating') {
          return (
              <div className="flex items-center gap-2 text-xs bg-green-900/30 border border-green-800 text-green-300 p-2 rounded w-full justify-center">
                  <TrendingUp className="w-4 h-4" />
                  <span>Dominating (Success Streak) - AI is harder</span>
              </div>
          );
      }
      return (
          <div className="flex items-center gap-2 text-xs bg-stone-800/50 border border-stone-700 text-stone-400 p-2 rounded w-full justify-center">
              <Minus className="w-4 h-4" />
              <span>Balanced (Normal Difficulty)</span>
          </div>
      );
  };
        ...
        <div className="bg-stone-900 border-2 border-amber-700 rounded-lg shadow-2xl w-full max-w-lg flex flex-col max-h-[85vh] animate-in zoom-in duration-200">
            ...
            <div className="flex flex-1 overflow-hidden mt-2">
                ...
                <div className="w-16 bg-stone-950 border-r border-stone-800 flex flex-col items-center py-4 gap-4 shrink-0">
                    ...
                    <div className="w-full h-px bg-stone-800 my-1" />
                    <button 
                        onClick={() => { setMode('vision'); setResult(""); }}
                        className={`p-2 rounded transition-colors ${mode === 'vision' ? 'bg-blue-900 text-blue-200' : 'text-stone-500 hover:text-blue-500'}`}
                        title="Сканер Мап (Vision)"
                    >
                ...
                <div className="flex-1 flex flex-col p-4 overflow-hidden bg-stone-900">
                    ...
                    <div className="mb-4 space-y-3">
                        ...
                        {mode === 'vision' && (
                            <div className="flex flex-col gap-4 text-center">
                                <div className="p-4 border-2 border-dashed border-stone-600 rounded bg-stone-900/50">
                                    <Camera className="w-12 h-12 text-stone-500 mx-auto mb-2" />
                                    <p className="text-xs text-stone-400 mb-2">Намалюйте карту на папері та сфотографуйте її.</p>
                                    
                                    <label className="cursor-pointer inline-flex items-center gap-2 bg-blue-800 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded text-sm transition-colors shadow-lg">
                                ...
                                <div className="text-[10px] text-stone-500 italic text-left p-2 bg-black/20 rounded">
                                    <strong className="text-amber-500">Як це працює:</strong> ШІ проаналізує фото, накладе віртуальну сітку 20x15 та автоматично створить стіни там, де ви намалювали темні лінії.
                                </div>
                            </div>
                        )}
                    ...
                    {/* Footer Actions */}
                    {(result || imageUrl) && (mode !== 'handout' && mode !== 'control' && mode !== 'vision') && (
