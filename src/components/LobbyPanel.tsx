...
const LobbyPanel: React.FC = () => {
  ...
  const myPeerId = peerService.myPeerId;
  ...
  const handleStartGame = () => {
      setAppPhase('game');
      peerService.broadcast({ type: 'START_GAME', payload: { timestamp: Date.now() } });
  };

  const handleToggleReady = () => {
      const newState = !amIReady;
      setPlayerReady(myPeerId, newState);
      if (mpMode === 'client') {
          peerService.sendToHost({ type: 'PLAYER_READY', payload: { peerId: myPeerId, isReady: newState } });
      }
  };
        ...
        <div className="border-t border-stone-800 pt-4">
            ...
            <div className="space-y-3">
                ...
                <div className="grid grid-cols-1 gap-2">
                    <button 
                        onClick={() => toggleSetting('fogEnabled')}
                        disabled={!isHost}
                    ...
                    <button 
                        onClick={() => toggleSetting('allowPvp')}
                        disabled={!isHost}
                    ...
                    >
                     ...
                     <button 
                        onClick={() => toggleSetting('publicRolls')}
                        disabled={!isHost}
