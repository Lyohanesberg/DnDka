...
import { Character, AbilityScores, DND_CLASSES, CLASS_PRESETS, DND_RACES, RACE_PRESETS, SpellSlots, ClassItem, SKILL_DEFINITIONS } from '../types';
...
interface CharacterSheetProps {
  ...
  mapTokens?: any[]; // For trade logic
  onTrade?: (recipientName: string, item: string) => void; // New: Trade Callback
  isMultiplayer?: boolean; // New: Hide Start Button if in MP Lobby
}

// Multiclass Spell Slot Calculator
const calculateMulticlassSlots = (classes: ClassItem[]): SpellSlots => {
    let effectiveLevel = 0;
    let warlockSlots: SpellSlots = { 1: {current:0, max:0} }; // Placeholder
    ...
    classes.forEach(c => {
        ...
        // Warlock Pact Magic is separate (Simplified: we just take the warlock level if present)
        if (warlock.some(w => className.includes(w))) {
             // Warlock Logic (Simplified standalone)
            let qty = 1;
            if (c.level >= 2) qty = 2;
            if (c.level >= 11) qty = 3;
            if (c.level >= 17) qty = 4;
    ...
    if (effectiveLevel === 0) {
        // Return warlock slots if any, or empty
...
const CharacterSheet: React.FC<CharacterSheetProps> = ({ 
  ...
  // Find adjacent players for trading
  const adjacentPlayers = React.useMemo(() => {
      ...
      return mapTokens.filter(t => {
          ...
          return dist <= 7.5; // Within 1.5 squares (adjacent + diagonals)
      });
  ...
  useEffect(() => {
      if (!character.classes || character.classes.length === 0) {
          if (character.class) {
              const preset = CLASS_PRESETS[character.class];
  ...
  const calculateStats = (className: string, raceName: string): AbilityScores => {
    ...
    const classPreset = CLASS_PRESETS[className];
    if (classPreset) {
    ...
    const racePreset = RACE_PRESETS[raceName];
  ...
  const updateCharacterDerivedStats = (primaryClass: string, raceName: string, stats: AbilityScores) => {
    const classPreset = CLASS_PRESETS[primaryClass];
    const racePreset = RACE_PRESETS[raceName];
    ...
    onChange({
        ...
        currency: { pp: 0, gp: 10, ep: 0, sp: 0, cp: 0 }, // Reset gold on class change
  ...
  const handleRaceChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
      ...
      if (character.class) {
          ...
          const racePreset = RACE_PRESETS[newRace];
          onChange({
              ...character,
              race: newRace,
              stats: newStats,
  ...
  const availableClassesToAdd = DND_CLASSES.filter(c => !character.classes?.some(existing => existing.name === c));
